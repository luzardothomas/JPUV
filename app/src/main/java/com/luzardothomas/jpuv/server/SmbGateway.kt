package com.luzardothomas.jpuv.server
...
class SmbGateway(private val context: Context) {
    ...
    // Objeto de bloqueo estricto para operaciones de escritura en SharedPreferences
    ...
    fun saveCreds(
        ...
        synchronized(prefsLock) {
            ...
            // Usamos serverId en todas las claves de SharedPreferences
            editor.putString("creds_$serverId", enc)
            editor.putString(keyNameFor(serverId), (serverName ?: cleanHost))
            editor.putInt(keyPortFor(serverId), port)

            editor.putStringSet(KEY_SERVER_IDS, currentIds)

            // ✅ Actualizar último server usado
            editor.putString(KEY_LAST_SERVER_ID, serverId)

            val success = editor.commit()

            if (success) {
                Log.i(tag, "✅ Saved SMB creds ATOMICALLY: id=$serverId host=$cleanHost setSize=${currentIds.size}")
            } else {
     ...
     */
    fun loadCreds(serverId: String): Pair<String, SmbCreds>? {
            ...
            override fun onServiceFound(service: NsdServiceInfo) {
                val resolveListener = object : NsdManager.ResolveListener {
                    ...
                    override fun onServiceResolved(serviceInfo: NsdServiceInfo) {
                        // --- CORRECCIÓN AQUÍ ---
                        // Usamos una forma compatible de obtener la dirección IP
                        val addr = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                            serviceInfo.hostAddresses.firstOrNull()
                        } else {
                            @Suppress("DEPRECATION")
                            serviceInfo.host // En versiones viejas se usa .host (InetAddress)
                        } ?: return
                        // -----------------------

                        val port = if (serviceInfo.port > 0) serviceInfo.port else 445

                        // Extraemos la IP de forma segura
                        val ipString = addr.hostAddress ?: return

                        val host = when (addr) {
                            is Inet4Address -> ipString
                            is Inet6Address -> ipString
                            else -> ipString
                        }
                ...
                // Envolvemos la llamada en una función o bloque que ignore las deprecaciones de NSD
                @Suppress("DEPRECATION")
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                    // Android 14+
                    // Intentamos la versión con Executor, silenciando el aviso de que ya hay una más nueva
                    nsd.resolveService(service, { it.run() }, resolveListener)
                } else {
            ...
            for (iface in ifaces) {
                ...
                for (addr in iface.inetAddresses.toList()) {
                    if (addr is Inet4Address && !addr.isLoopbackAddress) {
                        ...
                        if (ip?.startsWith("169.254.") == false) return ip
                    }
                }
            }
        } catch (_: Exception) {}
