...
class CardPresenter : Presenter() {
    ...
    override fun onUnbindViewHolder(viewHolder: Presenter.ViewHolder) {
        val holder = viewHolder as CardViewHolder
        val imageView = holder.cardView.mainImageView

        if (imageView != null) {
            val ctx = imageView.context
            // Se fija si el contexto en la actividad sigue vivo
            val isValidContext = if (ctx is android.app.Activity) {
                !ctx.isFinishing && !ctx.isDestroyed
            } else {
                true
            }

            if (isValidContext) {
                // Solo limpiar si la actividad sigue con vida
                Glide.with(ctx).clear(imageView)
            }
            // Por seguridad limpiar el drwwable manualmente
