...
/**
 * Importador de videos de Servidores locales y externos
 */

class ServerAutoImporter(
    ...
    fun run(toast: (String) -> Unit, onDone: (Int) -> Unit, onError: (String) -> Unit) {
        ...
        Thread {
            ...
            try {
                ...
                var localResolver: LocalCoverResolver? = null
                if (indexStr != null && seriesMapStr != null) {
                    localResolver = LocalCoverResolver(indexStr, seriesMapStr)
                } else {
                    Log.w("AutoImporter", "API Desactivada. Importando en Modo Rápido.")
                }
                ...
                for ((saga, list) in sagaMap) {
                    val itemSuffix = list.firstOrNull()?.suffix ?: "servidor"
                    list.sortWith(compareBy({ extractMovieSortKey(it.smbPath) },{it.title}))
    ...
    private fun processServer(
        ...
        for (p in smbFiles) {
            ...
            val placeholder = "android.resource://${context.packageName}/${R.drawable.movie}"
            val img = cover?.url?.trim().orEmpty().ifBlank { placeholder }
    ...
    private fun buildVideoUrl(serverId: String, share: String, path: String, isExternal: Boolean): String {
        ...
        val endsWithSlash = cleanPath.endsWith("/")
        val pathEnc = cleanPath.split("/")
            .filter { it.isNotEmpty() }
            .joinToString("/") {
                URLEncoder.encode(it, "UTF-8").replace("+", "%20")
            }
        val finalPath = if (endsWithSlash && pathEnc.isNotEmpty()) "$pathEnc/" else pathEnc
        val shareEnc = URLEncoder.encode(share, "UTF-8").replace("+", "%20")

        if(isExternal == false) {
            return "http://127.0.0.1:$proxyPort/smb/$serverId/$shareEnc/$finalPath"
        }
        ...
        val safePass = URLEncoder.encode(creds.password, "UTF-8").replace("+", "%20")
        val safeUser = URLEncoder.encode(creds.username, "UTF-8").replace("+", "%20")

        // Construcción de la URL dinámica
        val finalUrl = "http://$safeUser:$safePass@$host:$webDavPort/$shareEnc/$finalPath"
